<template>
  <el-container style="height: 100vh; background: linear-gradient(135deg, #0f172a, #1e293b);">
    ...
    <el-container>
      <!-- 侧边菜单 - 深蓝色科技感主题 -->
      <el-aside width="200px" style="border-right: 1px solid #2d3748; height: calc(100vh - 60px);">
        <el-menu
        ...
        >
          <template v-for="item in userStore.menuList" :key="item.id">
            ...
            <el-menu-item v-else :index="item.menuCode || 'default'">
      ...
      <!-- 主内容区域 - 修复撑满问题 -->
      <el-container style="height: calc(100vh - 60px); display: flex; flex-direction: column;">
        <el-main 
          style="
            background: #ffffff;
            color: #333333;
...
// 更新默认展开菜单
const updateDefaultOpeneds = () => {
  const openeds = userStore.menuList
...
/* 路由链接样式 */
.el-menu-item a, .el-sub-menu__title a {
  color: inherit;
...
/* 科技感光效 - 侧边菜单 */
.el-menu-item::after, .el-sub-menu__title::after {
  content: '';
  position: absolute;
  width: 3px;
  height: 100%;
...
.el-menu-item.is-active::after, .el-sub-menu__title.is-active::after {
  opacity: 1;
}

/* 悬停光效 */
.el-menu-item:hover::after, .el-sub-menu__title:hover::after {
  opacity: 0.7;
}
